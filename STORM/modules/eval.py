...
async def executor(client: STORMX, message: Message):
...
@STORMX.on_cb("runtime")
async def runtime_func_cq(_, cq):
    runtime = cq.data.split(None, 1)[1]
    await cq.answer(runtime, show_alert=True)


@STORMX.on_cb("forceclose")
...
async def shellrunner(client: STORMX, message: Message):
    if len(message.command) < 2:
